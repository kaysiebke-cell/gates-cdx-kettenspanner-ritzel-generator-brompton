...
export const i18n = {
  ...
  strings: {
    de: {
      ...
      spokes_wall: 'Kranz-Wand',
      spokes_round: 'Speichen-Rundung',
      spokes_hub_r: 'Nabenübergang',
      hint4: 'Spart Material im Steg zwischen Nabe und Zahnkranz: 0 = aus, sonst die Anzahl der Arme (z. B. 5). Schwung 0° = gerade Speichen, größere Werte biegen sie tangential. Sinnvoll ab 17 Zähnen — darunter ist der freie Ring zu schmal, dann bleibt der Steg automatisch voll. Passen Schwung oder Rundung nicht in den freien Ring, rasten die Felder nach der Eingabe auf den Wert ein, der tatsächlich geschnitten wird. Die Öffnungskanten werden an beiden Stirnflächen mit dem Wert aus Zahn-Rundung gebrochen. Mit Speichen weichst du vom Standard ab: die vorgebauten Release-Dateien passen dann nicht mehr, es braucht den Cloud-Bau.',
    ...
    en: {
      ...
      spokes_sweep: 'Sweep (°)',
      spokes_wall: 'Rim wall',
      spokes_round: 'Spoke Round',
      spokes_hub_r: 'Hub blend',
      hint4: 'Saves material in the web between hub and tooth rim: 0 = off, otherwise the number of arms (e.g. 5). Sweep 0° = straight spokes, larger values bend them tangentially. Worthwhile from 17 teeth up — below that the free ring is too narrow and the web stays solid automatically. If sweep or rounding do not fit the free ring, the fields snap to the value that is actually cut once you leave them. The opening edges are broken on both faces using the Tooth rounding value. Spokes take you off the standard values: the pre-built release files no longer match, so the cloud build is needed.',
