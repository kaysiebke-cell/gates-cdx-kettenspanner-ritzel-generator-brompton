...
// Renderbudget. Genau zwei Stufen, und beide entsprechen dem, was die
// Anwendung vorher schon gerechnet hat: „voll“ ist der bisherige Wert am
// PC, „sparsam“ der bisherige Wert auf Handy und Tablet. Bewusst KEINE
// Zwischenstufe für Tablets — eine höhere Last als bisher wäre auf genau
// den Geräten neu, die am wenigsten davon vertragen. Die Erkennung darf
// hier nur besser einordnen, nicht mehr verlangen.
// Die Kantenglättung steht nur beim Start zur Wahl (der WebGL-Kontext wird
// einmal erzeugt), Pixeldeckel und Schattenkarte lassen sich nachziehen.
const QUALITAET = {
  voll:    { kantenglaettung: true,  pixelDeckel: 2,   schattenKarte: 2048 },
...
// Fingerbedienung heißt Akku, passive Kühlung und ein mobiler Grafikkern —
// dort wird gespart, wie bisher. Zusätzlich stufen auch am PC wenige Kerne
// oder wenig Speicher herab (deviceMemory kennt nur Chromium, fehlt sonst
// und zählt dann nicht mit).
function leistungsklasse(form) {
  if (form !== 'desktop') return 'sparsam';
